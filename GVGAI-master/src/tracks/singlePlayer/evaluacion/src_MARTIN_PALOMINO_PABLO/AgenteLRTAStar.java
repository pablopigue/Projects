...
//Mismos comentarios que RTA pero unica diferencia coger el minimo en vez del segundo minimo
public class AgenteLRTAStar extends AbstractPlayer {
	...
	public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
	    ...
	    //Calculamos los costos solo para movimientos válidos
	    for (int i = 0; i < 4; i++) {
	    ...
	    //Encontramos el mejor movimiento
	    int mejorIdx = 0;
	    ...
	    //Actualizamos de acuerdo a LRTA 
	    double actual = getHeuristica(current_node.estado);
	    double nueva = Math.max(actual, min);
