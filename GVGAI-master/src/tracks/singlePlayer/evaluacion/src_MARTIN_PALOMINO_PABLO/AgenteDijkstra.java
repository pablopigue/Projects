...
public class AgenteDijkstra extends AbstractPlayer {
	...
	public List<Types.ACTIONS> plan; //plan a realizar (lista de acciones hacia meta devuelto por djikstra)
	...
	public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
		...
		if (plan == null || plan.isEmpty()) {
			...
			plan = Djikstra(estado_interno, matriz);
			if(!plan.isEmpty()) {
	...
	//Algoritmo Djikstra que devuelve el plan 
	public List<Types.ACTIONS> Djikstra(Estado inicio, ArrayList<Observation>[][] mapa) {
		...
		while (!frontier.isEmpty() && !SolutionFound) {
			...
			if (!SolutionFound) {
				...
				//Si no se ha explorado previamente se actualizan costo y orden  y lo añadimos a abiertos
				...
				//Hijo arriba (mismo codigo que derecha pero adaptado a accion izquierda)
				...
				//Hijo abajo (mismo codigo que derecha pero adaptado a accion izquierda)
		...
		//Calculamos e imprimimos el tiempo
		...
		if (SolutionFound) {
			...
			System.out.println("Tamaño de la ruta: " + current_node.lista.size() + " | Cantidad de nodos: "
					+ contador_nodos + " | Tiempo en act: " + tiempoTotalms );
		}
		return plan;
	}
	...
	// CAPA AZUL ITYPE 9
	// JUGADOR ITYPE 10
	//Funcion que me dice si una casilla es transitable
	...
	//Funcion que aplica el cambio de realizar una accion a un estado
