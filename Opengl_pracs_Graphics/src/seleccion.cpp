...
bool AplicacionIG::seleccion( int x, int y ) 
{
   ...
   if (id==0){
      cout << "No has pinchado obj seleccionable" << endl;
      return false;
   }
   ...
   Objeto3D* objetivo = nullptr;
   glm::mat4 *matriz = new mat4(1.0f);
   vec3 centro = vec3(0,0,0);
   if(raiz->buscarObjeto(id,*matriz,&objetivo,centro)){
      return objetivo->cuandoClick(centro);
   }
