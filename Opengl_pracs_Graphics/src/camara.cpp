...
void Camara3Modos::desplRotarXY( const float da, const float db )
{
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         // .....
         org_polares[0] += da/100;
         org_polares[1] += db/100;
      ...
      case ModoCam::prim_pers_despl :
      {
         ...
         // .....
         punto_atencion = punto_atencion + ((da/100)*eje[X])+((db/100)*eje[Y]);
...
void Camara3Modos::moverZ( const float dz )
{
   ...
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         // .....
         const float radio_min=0.1;
         const float ratio=0.06;

         org_polares[2]= radio_min +(org_polares[2]-radio_min)*pow((1+ratio),dz);
         org_cartesianas= Cartesianas(org_polares);
      ...
      case ModoCam::prim_pers_rot :
      {
         ...
         // nota: los ejes no cambian
         // .....
         punto_atencion=punto_atencion+((dz/100)*eje[Z]);
         break;
      }
      case ModoCam::prim_pers_despl :
      {
